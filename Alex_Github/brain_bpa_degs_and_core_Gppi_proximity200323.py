...
with open('new_brain_degs_ppi_andcore_dict.pickle', 'rb') as handle:
    new_brain_degs_ppi_andcore_dict = pk.load(handle)
...
with open('input/ppi_spl.pickle', 'rb') as handle:   #This file is too big for Github but it can be re-created by running PPI_spl.py
    spl = pk.load(handle)
...
brain_degs_ppi_and_core_components_proximity={}
for condition,genelist in new_brain_degs_ppi_andcore_dict.items():
    ...
    for dis in disgenet_gda_curated_score_multigenes.keys():
        brain_degs_ppi_and_core_components_proximity[condition,dis]=calculate_proximity(G_ppi,spl,genelist,disgenet_gda_curated_score_multigenes[dis],bpa_random_distance)


#Let's save it
with open('output/brain_degs_ppi_and_core_components_proximity_200323.pickle', 'wb') as handle:
    pk.dump(brain_degs_ppi_and_core_components_proximity, handle, protocol=pk.HIGHEST_PROTOCOL)
